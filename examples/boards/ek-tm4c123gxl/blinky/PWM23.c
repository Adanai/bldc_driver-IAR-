...
#define PWM_0_GENC_ACTCMPCD_ONE 0x0000C000  // Set the output signal to 1
#define PWM_0_GENC_ACTLOAD_ZERO 0x00000008  // Set the output signal to 0
...
/*********************************************************************/
// period is 16-bit number of PWM clock cycles in one period (3<=period)
// period for PB5 and PB4 must be the same
// duty is number of PWM clock cycles output is high  (2<=duty<=period-1)
// PWM clock rate = processor clock rate/SYSCTL_RCC_PWMDIV
//                = BusClock/2 
//                = 40 MHz/2 = 20 MHz (in this example)
...
void PWM0C_Init(uint16_t period, uint16_t duty){
  ...
  GPIO_PORTB_PCTL_R &= ~0x00F00000;     // configure PB6 as PWM0
  ...
  PWM0_0_CTL_R = 0;                     // 4) re-loading down-counting mode
  PWM0_0_GENA_R = 0xC8; 
  // low on LOAD, high on CMPC down
  // PB5 goes low on LOAD
  // PB5 goes high on CMPA down
  PWM0_0_LOAD_R = period - 1;           // 5) cycles needed to count down to 0
  PWM0_0_CMPA_R = duty - 1;              // 6) count value when output rises
  PWM0_0_CTL_R |= 0x00000001;           // 7) start PWM0
  PWM0_ENABLE_R |= 0x00000008;          // enable PB6/M0PWM0
}
